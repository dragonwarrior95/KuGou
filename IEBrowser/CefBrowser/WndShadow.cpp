...
// 			_RPTF2(_CRT_WARN, "%d %d\n", ptAnchorsOri[i][0], ptAnchorsOri[i][1]);
	...
	len = (2 * nKernelSize + 1) * (2 * nKernelSize + 1);
	UINT32 *pKernel = new UINT32[len];
	for(y = 0; y < len; y++)
	{
		pKernel[y] = 0;
	}
...
{
	if(NewSize > 20 || NewSize < -20 || m_hWnd == NULL)
		return false;
...
{
	if(NewSharpness > 20 || m_hWnd == NULL)
		return false;
...
{
	if(NewDarkness > 255 || m_hWnd == NULL)
		return false;
...
bool CWndShadow::SetPosition(int NewXOffset, int NewYOffset)
{
	if(NewXOffset > 20 || NewXOffset < -20 ||
		NewYOffset > 20 || NewYOffset < -20 || m_hWnd == NULL)
		return false;
